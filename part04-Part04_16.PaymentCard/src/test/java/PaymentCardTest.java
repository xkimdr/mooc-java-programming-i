...
public class PaymentCardTest {
    ...
    public void testConstructor() throws Throwable {
        ...
        assertTrue("Define for the class " + klassName + " a public constructor: public " + klassName
                + "(double initialBalance)", ctor.isPublic());
        ctor.invoke(4.0);
    }
    ...
    public void toStringInitiallyCorrect() throws Throwable {
        ...
        assertFalse(
                "Give the " + className + " class the method public String toString() as instructed in the exercise",
                result.contains("@"));
        assertEquals("created card =  new " + className + "(" + sum + "); the method toString doesn't work properly:",
    ...
    public void methodEatAffordablyExists() throws Throwable {
        ...
        String v = "\nThe code that caused the error " + className + " lk = new " + className
                + "(4.0); lk.eatAffordably()";
    ...
    public void eatAffordablyDecreasesBalance() throws Throwable {
        ...
        assertEquals("the method eatAffordably doesn't seem to decrease the balance on the card correctly. \n"
                + "Check that the following works: card = new " + className
                + "(6); card.eatAffordably(); System.out.println(card);\n",
                expected.toLowerCase(), result.toLowerCase());
        ...
        assertEquals("the method eatAffordably doesn't seem to decrease the balance on the card correctly. \n"
                + "Check that the following works: card = new " + className
                + "(6); card.eatAffordably(); card.eatAffordably(); System.out.println(card);\n",
                expected.toLowerCase(), result.toLowerCase());
    }
    ...
    public void methodEatHeartilyExists() throws Throwable {
        ...
        String e = "\nThe code that cause the error: " + className + " lk = new " + className
                + "(4.0); lk.eatHeartily()";
    ...
    public void eatHeartilyDecreasesBalance() throws Throwable {
        ...
        assertEquals("the method eatHeartily doesn't seem to decrease the balance correctly. \n"
                + "Check that the following works: card = new " + className
                + "(10); card.eatHeartily(); System.out.println(card);\n", expected.toLowerCase(),
                result.toLowerCase());
        ...
        assertEquals("the method eatHeartily doesn't seem to decrease the balance correctly. \n"
                + "Check that the following works: card = new " + className
                + "(10); card.eatHeartily(); card.eatHeartily(); System.out.println(card);\n", expected.toLowerCase(),
                result.toLowerCase());
    }
    ...
    public void eatAffordablyWillNotCauseNegativeBalance() throws Throwable {
        ...
        assertEquals(
                "The balance should not become negative if an affordable meal cannot be afforded. Check that the following works: \n"
                        + "card = new " + className
                        + "(2); card.eatAffordably(); card.eatAffordably(); System.out.println(card);\n",
                expected.toLowerCase(), result.toLowerCase());
    }
    ...
    public void eatHeartilyWillNotCauseNegativeBalance() throws Throwable {
        ...
        assertEquals(
                "The balance should not become negative if a hearty meal cannot be afforded. Check that the following works: \n"
                        + "card = new " + className
                        + "(7); card.eatHeartily(); card.eatHeartily(); System.out.println(card);\n",
                expected.toLowerCase(), result.toLowerCase());
    }
    ...
    public void allMoneyCanBeUsedToEat() throws Throwable {
        ...
        assertEquals("If the balance on the card is " + HEARTY + ", you should be able to use it to eat heartily. \n"
                + "Check that the following works: card = new " + className + "(" + HEARTY
                + "); card.eatHeartily();  System.out.println(card);", expected.toLowerCase(), result.toLowerCase());
    }
    ...
    public void allMoneyCanBeUsedToEat2() throws Throwable {
        ...
        assertEquals(
                "If the balance on the card is " + AFFORDABLE + ", you should be able to use it to eat affordably. \n"
                        + "Check that the following works: card = new " + className + "(" + AFFORDABLE
                        + "); card.eatAffordably();  System.out.println(card);",
                expected.toLowerCase(), result.toLowerCase());
    }
    ...
    public void methodAddMoneyExists() throws Throwable {
        ...
        assertTrue("give the class " + klassName + " the method public void " + method + "(double amount) ",
                klass.method(object, method)
    ...
    public void addingNegativeAmountDoesntChangeBalance() throws Throwable {
        ...
        assertEquals("Adding a negative amount should have no effect. Check that the following works:\n"
                + "card = new " + className
                + "(10); card.addMoney(-7); card.addMoney(1); card.AddMoney(-3);System.out.println(card);\n",
                expected.toLowerCase(), result.toLowerCase());
    }
    ...
    public void multipleCards() {
        ...
        for (String row : rows) {
            assertTrue(
                    "Print the owner of the card at the beginning of the row in addition to the card information! Remember to delete any extra code from the main program when doing the exercise 04-16.6",
                    row.toLowerCase().contains("paul") || row.toLowerCase().contains("matt"));
            assertFalse("Print only one card's information on a single row, now the following row is printed: " + row,
                    row.toLowerCase().contains("paul") && row.toLowerCase().contains("matt"));
        }
    ...
    private void sanityCheck() throws SecurityException {
        ...
        for (Field field : fields) {
            assertFalse("you don't need \"static variables\", remove " + field(field.toString()),
                    field.toString().contains("static") && !field.toString().contains("final"));
            assertTrue("the visibility of all object variables must be private, but the code contains: "
                    + field(field.toString()), field.toString().contains("private"));
        }
        ...
        if (fields.length > 1) {
            ...
            assertTrue(
                    "class " + className
                            + " only needs an object variable that stores the balance, remove the extra variables",
                    var < 2);
        }
    }
