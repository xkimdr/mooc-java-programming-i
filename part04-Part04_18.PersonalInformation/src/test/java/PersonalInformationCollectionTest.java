...
public class PersonalInformationCollectionTest {
    ...
    public void classPersonalInformationHasNotBeenModified() {
        ...
        assertEquals("You've modified the class PersonalInformation. Revert the changes you've made.",
                "last, first (id)", info.toString());
    }

    @Test
    public void testInputFirst() {
        List<PersonalInformation> people = new ArrayList<>();
        people.add(new PersonalInformation("Jean", "Bartik", "271114"));
    ...
    private void enterAndCheck(List<PersonalInformation> people) {
        ...
        for (PersonalInformation p : people) {
            assertFalse(
                    "When the input is\n" + input + "\n, the output of the program should not contain \""
                            + p.getIdentificationNumber() + "\". The output was:\n" + io.getSysOut(),
                    io.getSysOut().contains(p.getIdentificationNumber()));
        }

        for (PersonalInformation p : people) {
            assertTrue(
                    "When the input is\n" + input + "\n, the output of the program should contain \"" + p.getFirstName()
                            + " " + p.getLastName() + "\". The output was:\n" + io.getSysOut(),
                    io.getSysOut().contains(p.getFirstName() + " " + p.getLastName()));
        }

        for (PersonalInformation p : people) {
            String n = p.getFirstName() + " " + p.getLastName();
            assertTrue("When the input is\n" + input + "\n, the string \"" + n
                    + "\" must appear on its own row (with the row containing nothing else). The output was:\n"
    ...
    private void callMain(Class kl) {
        try {
            ...
            fail("Something weird occurred. It could be that the void main(String[] args) method of the class " + kl
                    + " has disappeared\n"
